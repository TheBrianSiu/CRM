...
router.get("/sales-records", async (req, res) => {
  ...
  const hasPermission = await retreieve_user_permission(
    userid,
    "read:saleschart"
  );
...
router.get("/est-sales-records", async (req, res) => {
  ...
  db.query(sql, [start, end], (err, data) => {
    if (err) {
      return res
        .status(500)
        .json({ message: "Error retrieving est sales value data" });
    }
...
router.get("/project-records", async (req, res) => {
  const start = req.query.start;
  const end = req.query.end;
  const sql = `
...
//count project number
router.get("/closed-project-records", async (req, res) => {
  const start = req.query.start;
  const end = req.query.end;
...
//count customer
router.get("/customers-records", async (req, res) => {
  const start = req.query.start;
  const end = req.query.end;
  const sql = `
  ...
  db.query(sql, [start, end], (err, data) => {
    if (err) {
      return res
        .status(500)
        .json({ message: "Error retrieving customer data" });
    }
...
router.get("/monthly-project", async (req, res) => {
  const sql = `
  SELECT
...
router.get("/monthly-clients", async (req, res) => {
